...
const SideNav = ({links, show, setShow}) => {
    ...
    return(
        <div className={`fixed w-full h-full top-0 left-0 ${show ? "" : "hidden"}`}>
            <div className={`w-full h-full bg-gray-500  opacity-70 ${show ? "transition ease-in-out delay-150 scale-100 duration-300" : "hidden"}`} onClick={close}>  </div>
            ...
            <div className={`fixed top-0 left-0 bg-white h-full w-64 p-4 transition duration-300 delay-150  ease-in ${show ? "scale-x-100" : "scale-x-0"}`}>
                ...
                <button className={`${styles.connectBtn} w-full h-12 rounded-lg px-8`}> Connect Wallet </button>
